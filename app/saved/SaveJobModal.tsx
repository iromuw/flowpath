...
export function SaveJobModal({ onClose, onCreated }: Props) {
  ...
  return (
      ...
      footer={
        <>
          ...
          <button
            ...
            form="save-job-form"
            disabled={loading}
            className="px-4 py-2 text-sm font-medium text-white bg-[#0FA878] rounded-lg hover:bg-[#0D9068] transition-colors disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
          >
            {loading && (
    ...
    >
      <form id="save-job-form" onSubmit={handleSubmit} className="space-y-4">
        {error && (
